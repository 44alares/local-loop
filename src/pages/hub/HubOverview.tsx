...
const benefitCards = [{
  icon: ShoppingCart,
  title: 'Group Buys',
  desc: 'Pool orders with nearby makers and get better prices on materials.',
  anchor: '#group-buys'
}, {
  icon: Tag,
  title: 'Community Deals',
  desc: 'Discounts and offers we collect from suppliers — grab what helps you.',
  anchor: '#deals'
}, {
  icon: Building2,
  title: 'Supplier Contacts',
  desc: 'Vetted suppliers with fair terms we negotiated for the community.'
}, {
  icon: BookOpen,
  title: 'Guides & Resources',
  desc: 'Practical stuff: material guides, pricing help, packaging tips.'
}, {
  icon: Users,
  title: 'Events & Workshops',
  desc: 'Online meetups, Q&As with suppliers, and the occasional print clinic.'
}, {
  icon: Headphones,
  title: 'Direct Help',
  desc: 'Got a question? Just ask. We actually reply.'
}];
...
export default function HubOverview() {
  ...
  return <div className="animate-fade-in">
      ...
      {role && <section className="py-12 bg-secondary/5">
          <div className="container max-w-4xl">
            <h2 className="text-xl font-semibold mb-6">What's useful for you as a {role}</h2>
            <div className="grid sm:grid-cols-2 gap-3">
              {roleBenefitHighlights[role].map((b, i) => <div key={i} className="flex items-start gap-3 rounded-lg border border-secondary/20 bg-background p-4">
      ...
      <section className="py-16">
        <div className="container">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {benefitCards.map(card => <Card key={card.title} className="card-hover">
                <CardContent className="p-6 space-y-4">
                  ...
                  {card.anchor && <Button variant="link" className="p-0 h-auto text-secondary" asChild>
      ...
      <section className="py-16 bg-muted/30">
        <div className="container max-w-3xl">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            ...
            desc: 'Pick your role and region so we show you relevant stuff.'
          }, {
            step: '02',
            title: 'Grab what helps',
            desc: 'Browse deals and group buys. Join what makes sense for you.'
          }, {
            step: '03',
            title: 'Better terms, together',
            desc: 'When enough people join, we all get better prices. Simple.'
          }].map(s => <div key={s.step} className="text-center space-y-3">
      ...
      <section id="deals" className="py-16 bg-muted/30">
        <div className="container max-w-3xl space-y-6">
          <div className="space-y-2">
            <h2 className="text-xl md:text-2xl font-semibold">Deals you can grab (beta example) </h2>
            <p className="text-sm text-muted-foreground">
              Offers from suppliers and partners. Claim what's useful, ignore the rest.
            </p>
          </div>

          {filteredDeals.length === 0 ? <div className="rounded-lg border border-dashed border-border p-8 text-center space-y-3">
            ...
            const regionLbls = deal.regionIds.map(id => regions.find(r => r.id === id)?.label).filter(Boolean).join(', ');
            return <div key={deal.id} className="rounded-lg border border-border bg-background p-4 flex flex-col sm:flex-row sm:items-center gap-3">
      ...
      <section id="group-buys" className="py-16">
        <div className="container max-w-3xl space-y-6">
          <div className="space-y-2">
            <h2 className="text-xl md:text-2xl font-semibold">Group buys (beta example) </h2>
            <p className="text-sm text-muted-foreground">
          ...
          {filteredGroupBuys.length === 0 ? <div className="rounded-lg border border-dashed border-border p-8 text-center space-y-3">
            ...
            const regionLbls = gb.regionIds.map(id => regions.find(r => r.id === id)?.label).filter(Boolean).join(', ');
            return <div key={gb.id} className="rounded-lg border border-border bg-background p-4 space-y-3">
        ...
        <div className="container max-w-xl space-y-6">
          ...
          <div className="rounded-lg border border-border bg-background p-6 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea placeholder="e.g. Bulk PETG in Berlin, nozzle deals in LA, packaging supplies..." value={requestText} onChange={e => setRequestText(e.target.value)} rows={3} />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">Email (optional)</label>
              <Input type="email" placeholder="So we can let you know if it happens" value={requestEmail} onChange={e => setRequestEmail(e.target.value)} />
            </div>
