...
const roles = [
  ...
  { id: 'follower', label: 'Just following the project' },
];

const printingTypes = ['FDM', 'Resin', 'Both'];
const makerMaterials = ['PLA', 'PETG', 'Nylon', 'Resin'];
const capacityOptions = [
  { value: 'occasional', label: 'Occasional (1–2 prints/week)' },
  { value: 'regular', label: 'Regular (several prints/week)' },
        ...
        <div className="container">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
              Zero commitment signup to map country + ZIP/postal code and validate MakeHug's local viability. 
              First 100 makers/designers: <strong>0% platform fee for 6 months</strong>, permanent Founder badge, 
                  ...
                  <CardContent className="space-y-4">
                      ...
                      <div className="flex gap-2">
                        {printingTypes.map((type) => (
                          <button
                            key={type}
                            type="button"
                            onClick={() => toggleArrayItem(printingType, setPrintingType, type)}
                            className={`px-4 py-2 rounded-lg border text-sm transition-all ${
                              printingType.includes(type)
                                ? 'border-secondary bg-secondary/10 text-secondary'
                                : 'border-border hover:border-secondary/50'
                            }`}
                          >
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {makerMaterials.map((mat) => (
                          <button
                            key={mat}
                            type="button"
                            onClick={() => toggleArrayItem(materials, setMaterials, mat)}
                            className={`px-4 py-2 rounded-lg border text-sm transition-all ${
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() => { setCanHandoff(false); setHandoffRadius(''); }}
                          className={`px-4 py-2 rounded-lg border text-sm transition-all ${
                            canHandoff === false
                              ? 'border-secondary bg-secondary/10 text-secondary'
                              : 'border-border hover:border-secondary/50'
                    ...
                    {canHandoff && (
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                          {handoffRadiusOptions.map((radius) => (
                            <button
                              key={radius}
                              type="button"
                              onClick={() => setHandoffRadius(radius)}
                              className={`px-4 py-2 rounded-lg border text-sm transition-all ${
                                handoffRadius === radius
                                  ? 'border-secondary bg-secondary/10 text-secondary'
                                  : 'border-border hover:border-secondary/50'
                              }`}
                            >
                    ...
                    <div className="space-y-2">
                      ...
                      <Input 
                        id="portfolio" 
                        placeholder="Instagram / Etsy / Printables / website"
                        value={portfolioLink}
                      ...
                      <div className="flex gap-2">
                        {designTypes.map((type) => (
                          <button
                            key={type}
                            type="button"
                            onClick={() => toggleArrayItem(designType, setDesignType, type)}
                            className={`px-4 py-2 rounded-lg border text-sm transition-all ${
                              designType.includes(type)
                                ? 'border-secondary bg-secondary/10 text-secondary'
                                : 'border-border hover:border-secondary/50'
                            }`}
                          >
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {[...buyerInterests, 'Other'].map((interest) => (
                          <button
                            key={interest}
                            type="button"
                            onClick={() => toggleArrayItem(buyerInterest, setBuyerInterest, interest)}
                            className={`p-2 rounded-lg border text-sm text-left transition-all ${
                              buyerInterest.includes(interest)
                                ? 'border-secondary bg-secondary/10 text-secondary'
                                : 'border-border hover:border-secondary/50'
                            }`}
                          >
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        {payMoreOptions.map((opt) => (
                          <button
                            key={opt}
                            type="button"
                            onClick={() => setPayMore(opt)}
                            className={`px-4 py-2 rounded-lg border text-sm transition-all ${
                              payMore === opt
                                ? 'border-secondary bg-secondary/10 text-secondary'
                                : 'border-border hover:border-secondary/50'
                      ...
                      {payMore === 'Depends' && (
                        <Textarea 
                          placeholder="What does it depend on?"
                          value={payMoreDepends}
                          onChange={(e) => setPayMoreDepends(e.target.value)}
                          className="mt-2"
                        />
                      )}
                    </div>
